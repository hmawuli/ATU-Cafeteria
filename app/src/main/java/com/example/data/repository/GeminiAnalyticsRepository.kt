...
class GeminiAnalyticsRepository {
    ...
    suspend fun generateHistoricalOrderInsights(
        ...
        if (apiKey.isEmpty() || apiKey == "MY_GEMINI_API_KEY") {
            return@withContext "### 📈 ATU Intelligence: Historical Order Analytics Summary for **$vendorName**\n" +
    ...
    suspend fun generateDailyDemandForecast(
        ...
        if (apiKey.isEmpty() || apiKey == "MY_GEMINI_API_KEY") {
            return@withContext "### 🔮 ATU Daily Demand Forecast for **$vendorName**\n" +
                    "*(Offline Forecast — Active Predictive Models Running)*\n\n" +
    ...
    suspend fun generateStudentMenuRecommendations(
        ...
        if (apiKey.isEmpty() || apiKey == "MY_GEMINI_API_KEY") {
            return@withContext "### 🧠 ATU Student Menu Recommendations for **$studentName**\n" +
                    "*(Local Recommendation Engine — Live Campus Food Models)*\n\n" +
    ...
    suspend fun predictLowStockItems(
        ...
        if (apiKey.isEmpty() || apiKey == "MY_GEMINI_API_KEY") {
            ...
            return@withContext "### 🔮 ATU Low-Stock Predictive Predictions for $vendorName\n\n" + predictions.joinToString("\n\n")
        }
    ...
    suspend fun analyzeCartNutritionalContent(
        ...
        val apiKey = BuildConfig.GEMINI_API_KEY
        if (apiKey.isEmpty() || apiKey == "MY_GEMINI_API_KEY") {
            return@withContext "### 🥗 ATU Cart Health Advisor\n" +
