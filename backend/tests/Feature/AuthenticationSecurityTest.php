...
class AuthenticationSecurityTest extends TestCase
{
    ...
    public function test_enabled_admin_requires_two_factor_before_token_is_issued(): void
    {
        ...
        $response = $this->postJson('/api/login', ['username' => $admin->username, 'pin' => hash('sha256', '1234')]);
    ...
    public function test_password_reset_revokes_existing_tokens(): void
    {
        ...
        $code = null;
        $notification = null;
        Notification::assertSentTo($user, AuthenticationCodeNotification::class, function ($notificationInstance) use (&$notification) { $notification=$notificationInstance; return true; });
        $ref = new \ReflectionClass($notification); $prop=$ref->getProperty('code'); $prop->setAccessible(true); $code=$prop->getValue($notification);
        $this->postJson('/api/password/reset',['username'=>$user->username,'code'=>$code,'pin'=>hash('sha256','5678')])->assertOk();
        $this->assertDatabaseCount('personal_access_tokens',0);
        $this->assertTrue(Hash::check(hash('sha256','5678'),$user->fresh()->password));
    }
}
