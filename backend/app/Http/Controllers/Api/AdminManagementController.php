...
class AdminManagementController extends Controller
{
    ...
    public function users(Request $request)
    {
        ...
        if ($request->filled('role')) {
            $query->where('role', strtoupper($request->string('role')));
        }
        if ($request->filled('status')) {
            $query->where('account_status', strtoupper($request->string('status')));
        }
    ...
    public function updateAdminLevel(Request $request, User $user)
    {
        ...
        if ($user->role !== 'ADMIN') {
            return response()->json(['success' => false, 'message' => 'Target user is not an administrator.'], 422);
        }
        $data = $request->validate(['admin_level' => ['required', Rule::in(['SUPER_ADMIN', 'CAFETERIA_ADMIN', 'FINANCE_ADMIN'])]]);
        if ($user->id === $request->user()->id && $data['admin_level'] !== 'SUPER_ADMIN') {
            return response()->json(['success' => false, 'message' => 'You cannot lower your own Super Admin privileges.'], 422);
        }
    ...
    public function orders(Request $request)
    {
        ...
        if ($request->filled('student_id')) {
            $query->where('user_id', $request->integer('student_id'));
        }
