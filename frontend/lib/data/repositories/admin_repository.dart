...
class AdminRepository {
  ...
  Future<Map<String, dynamic>> dashboard() async =>
      _map(await api.request('GET', 'admin/dashboard'));
  ...
  Future<Map<String, dynamic>> vendors() async =>
      _map(await api.request('GET', 'admin/vendors'));
  Future<Map<String, dynamic>> createVendor(
      ...
      _map(await api.request('POST', 'admin/vendors', body: {
        'email': email,
        'password': password,
        'fullName': fullName,
        'storeName': storeName,
  ...
  Future<Map<String, dynamic>> orders({String? status}) async => _map(
  ...
  Future<Map<String, dynamic>> settlements() async =>
      _map(await api.request('GET', 'admin/settlements'));
  ...
  Future<Map<String, dynamic>> finalizeSettlementPayout(int id, String otp) async =>
      _map(await api.request(
        'POST',
        'admin/settlements/$id/payout/finalize',
        idempotencyKey: ApiClient.newIdempotencyKey(),
        body: {'otp': otp},
      ));
  ...
  Future<Map<String, dynamic>> settings() async =>
      _map(await api.request('GET', 'admin/settings'));
  Future<Map<String, dynamic>> updateUserStatus(int id, String status) async =>
      _map(await api.request('PATCH', 'admin/users/$id/status',
          body: {'account_status': status}));
  Future<Map<String, dynamic>> updateVendorStatus(
          int id, String status) async =>
      _map(await api.request('PATCH', 'admin/vendors/$id/status',
          body: {'operational_status': status}));
  Future<Map<String, dynamic>> updateAdminLevel(int id, String level) async =>
      _map(await api.request('PATCH', 'admin/users/$id/admin-level',
          body: {'admin_level': level}));
  Future<Map<String, dynamic>> updateSetting(String key, String value) async =>
      _map(await api.request(
          'PUT', 'admin/settings/${Uri.encodeComponent(key)}',
          body: {'value': value}));
  Future<Map<String, dynamic>> walletAdjustment(
          int id, double amount, String type, String reason) async =>
      _map(await api.request(
          'POST', 'admin/finance/users/$id/wallet-adjustment',
          body: {'amount': amount, 'type': type, 'reason': reason}));

  Map<String, dynamic> _map(dynamic value) =>
      value is Map<String, dynamic> ? value : <String, dynamic>{'data': value};
