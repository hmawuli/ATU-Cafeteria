...
class _DefenseModeScreenState extends State<DefenseModeScreen> {
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      setState(() {
        if (user == null) {
          _error = 'Invalid defense account or PIN.';
        } else {
          _user = user;
        }
      });
    } catch (e) {
      if (mounted) setState(() => _error = 'Local database error: $e');
    } finally {
  ...
  Widget build(BuildContext context) {
    if (_user != null) {
      return _DefenseDashboard(user: _user!, onLogout: _logout);
    }
...
class _DefenseDashboardState extends State<_DefenseDashboard> {
  ...
  Future<void> _load() async {
    ...
    try {
      final foods = await _api.foods();
      final orders = await _api.ordersFor(_userId, _role);
      final balance = await _api.balance(_userId);
      final users = _role == 'ADMIN' ? await _api.users() : <Map<String, dynamic>>[];
      final reviews = await _api.reviews();
      final logs = await _api.auditLogs();
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Local database error: $e')),
      );
    }
  }
  ...
  Future<void> _placeOrder(Map<String, dynamic> food) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Order placed successfully. Wallet updated locally.'),
          ),
        );
      }
    } catch (e) {
  ...
  Future<void> _advanceOrder(Map<String, dynamic> order) async {
    ...
    final next = current == 'ORDER_PLACED'
        ? 'PREPARING'
        : current == 'PREPARING'
            ? 'READY_FOR_PICKUP'
        ...
        content: Column(
          ...
          children: [
            ValueListenableBuilder<int>(
              ...
              builder: (_, value, __) => DropdownButton<int>(
                value: value,
                isExpanded: true,
                items: List.generate(
                  5,
                  (i) => DropdownMenuItem(
                    value: i + 1,
                    child: Text('${i + 1} / 5 stars'),
                  ),
                ),
                onChanged: (v) {
                  if (v != null) rating.value = v;
                },
              ),
            ),
            TextField(
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              gradient: const LinearGradient(
                colors: [Color(0xFF063B82), Color(0xFF0B5BB8)],
              ),
            ),
            child: Column(
          ...
          const SizedBox(height: 18),
          if (student) ...[
            _sectionTitle('Available food'),
            ..._foods.map(_foodCard),
          ],
          if (!student) ...[
            _sectionTitle(vendor ? 'Incoming orders' : 'System overview'),
  ...
  Widget _orderCard(Map<String, dynamic> order) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          'Qty ${order['quantity']} • GH₵ ${total.toStringAsFixed(2)}\n'
          'Status: ${order['status']}\n'
          'Pickup PIN: ${order['pickup_pin']}',
        ),
        trailing: _role == 'VENDOR' && order['status'] != 'COMPLETED'
            ? IconButton(
                onPressed: () => _advanceOrder(order),
                icon: const Icon(Icons.arrow_forward_rounded),
              )
            : _role == 'STUDENT' && order['status'] == 'COMPLETED'
                ? IconButton(
        ...
        children: [
          ...
          ...data.map(
            (row) => Card(
              child: ListTile(
                leading: Icon(
                  _role == 'ADMIN' ? Icons.security : Icons.star,
                ),
                title: Text(
                  _role == 'ADMIN'
                      ? row['action'].toString()
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: [
          ...
          NavigationDestination(
            icon: Icon(
              _role == 'ADMIN' ? Icons.security_outlined : Icons.history,
            ),
            selectedIcon: Icon(
              _role == 'ADMIN' ? Icons.security : Icons.history,
            ),
            label: _role == 'ADMIN' ? 'Audit' : 'Activity',
          ),
        ],
      ),
    );
