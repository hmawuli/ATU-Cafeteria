...
class _CheckoutScreenState extends State<CheckoutScreen> {
      ...
      body: cart.isEmpty
          ...
          : ListView(
              ...
              children: [
                ...
                const Text('Confirm your meals and payment method before placing the order.'),
                ...
                Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  RadioGroup<String>(groupValue: _method, onChanged: (value) => setState(() => _method = value!), child: const Column(children: [
                    RadioListTile<String>(value: 'Wallet', title: Text('ATU Cafeteria Wallet'), secondary: Icon(Icons.account_balance_wallet_outlined)),
                    RadioListTile<String>(value: 'Online', title: Text('Mobile Money / Card'), subtitle: Text('Pay securely through Paystack.'), secondary: Icon(Icons.payments_outlined)),
                  ])),
                ]))),
                const SizedBox(height: 18),
                Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  const Text('Use loyalty points', style: TextStyle(fontWeight: FontWeight.w800)),
                  const SizedBox(height: 8),
                  const Text('10 points = GH₵ 1.00 discount.'),
                  const SizedBox(height: 8),
                  TextField(controller: _pointsController, keyboardType: TextInputType.number, onChanged: (_) => setState(() {}), decoration: const InputDecoration(labelText: 'Points to redeem', prefixIcon: Icon(Icons.stars_outlined))),
                ]))),
                const SizedBox(height: 18),
                Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  const Text('Order note (optional)', style: TextStyle(fontWeight: FontWeight.w800)),
                  const SizedBox(height: 8),
                  TextField(controller: _noteController, maxLength: 160, decoration: const InputDecoration(hintText: 'Add a note for the cafeteria', prefixIcon: Icon(Icons.notes_outlined))),
                ]))),
                const SizedBox(height: 18),
  ...
  Widget _fulfilmentCard(BuildContext context) => Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    const Text('Pickup options', style: TextStyle(fontWeight: FontWeight.w800)),
    RadioGroup<String>(groupValue: _fulfilment, onChanged: (value) => setState(() => _fulfilment = value!), child: const Column(children: [
  ...
  Future<void> _payOnline(CafeteriaProvider auth, CartProvider cart, int points, double finalTotal) async {
    ...
    if (email == null || email.trim().isEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('A valid email address is required for online payment.')));
      return;
    }
    setState(() => _submitting = true);
    try {
      final init = await auth.initializePaystackPayment(amount: finalTotal, email: email.trim(), purpose: 'DIRECT_ORDER_PAY');
      ...
      if (reference == null || authorizationUrl == null) throw Exception('Payment gateway returned an incomplete response.');
      ...
      if (!simulated) {
        ...
        if (!mounted) return;
        final verified = await showDialog<bool>(context: context, barrierDismissible: false, builder: (ctx) => AlertDialog(
          title: const Text('Complete your payment'),
          content: const Text('Finish the payment in the browser, then return here and tap “I have paid”.'),
          actions: [TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel')), FilledButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('I have paid'))],
        ));
        ...
        final proceed = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
          ...
          content: Text('The backend is using its local Paystack simulation. Continue as a simulated successful payment of GH₵ ${finalTotal.toStringAsFixed(2)}?'),
          actions: [TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel')), FilledButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('Simulate payment'))],
        ));
        if (proceed != true || !mounted) return;
      }
      final confirmed = await auth.verifyPaystackPayment(reference: reference, amount: finalTotal, purpose: 'DIRECT_ORDER_PAY');
      ...
      if (token == null || token.isEmpty) throw Exception('Your login session has expired. Please sign in again.');
      _api.token = token;

      final result = await _api.post('/student/cart-checkout', body: {
        'items': cart.toCheckoutPayload(), 'payment_method': 'momo', 'payment_reference': reference,
      ...
      await showDialog<void>(context: pageContext, builder: (ctx) => AlertDialog(
        ...
        content: Text(pickupPin == null ? 'Your order has been placed successfully.' : 'Order placed successfully. Pickup PIN: $pickupPin'),
        actions: [FilledButton(onPressed: () { Navigator.pop(ctx); if (orderId != null) { Navigator.pushReplacementNamed(context, '/order-tracking', arguments: orderId is int ? orderId : int.tryParse(orderId.toString())); } else { Navigator.pushReplacementNamed(context, '/student'); } }, child: const Text('View order'))],
      ));
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _showConfirmation(BuildContext context) async {
    ...
    if (auth.currentUser == null) {
      final goToLogin = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
        icon: const Icon(Icons.lock_outline, size: 42), title: const Text('Sign in required'),
        content: const Text('Please sign in to place your order. Your cart will be kept while you sign in.'),
        actions: [TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Not now')), FilledButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('Sign in'))],
      ));
      if (goToLogin == true && context.mounted) Navigator.pushNamed(context, '/login');
      return;
    }
    ...
    if (token == null || token.isEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Your login session is missing. Please sign in again.')));
      return;
    }
    _api.token = token;
    ...
    if (points > 0) {
      try { await _api.post('/student/loyalty/preview-discount', body: {'points_to_redeem': points}); }
      on ApiException catch (e) { if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.message))); return; }
    }
    if (_method == 'Online') {
      if (!mounted) return;
    ...
    final ok = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
      title: const Text('Confirm order'), content: Text('Place this order for GH₵ ${finalTotal.toStringAsFixed(2)} using your cafeteria wallet?'),
