...
class _CheckoutScreenState extends State<CheckoutScreen> {
          ...
          child:
            ...
            RadioGroup<String>(
                ...
                child: const Column(children: [
                  RadioListTile<String>(
                      value: 'Pickup now',
                      title: Text('Pickup when ready'),
                      subtitle: Text(
                          'Collect from the vendor when your order is ready.'),
                      secondary: Icon(Icons.storefront_outlined)),
                  RadioListTile<String>(
                      value: 'Schedule pickup',
                      title: Text('Schedule pickup'),
                      subtitle: Text('Choose a future pickup time.'),
            ...
            if (_fulfilment == 'Schedule pickup')
              OutlinedButton.icon(
                  ...
                  label: Text(_scheduledPickup == null
                      ? 'Choose pickup time'
                      : _formatDateTime(_scheduledPickup!))),
          ])));
  ...
  Future<void> _pickSchedule() async {
    ...
    final date = await showDatePicker(
        ...
        initialDate: _scheduledPickup ?? now);
    if (date == null || !mounted) return;
    final time = await showTimePicker(
        context: context,
        initialTime: _scheduledPickup == null
            ? TimeOfDay.fromDateTime(now.add(const Duration(hours: 1)))
            : TimeOfDay.fromDateTime(_scheduledPickup!));
  ...
  Future<void> _payOnline(CafeteriaProvider auth, CartProvider cart, int points,
      double finalTotal) async {
    ...
    if (email == null || email.trim().isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content:
              Text('A valid email address is required for online payment.')));
      return;
    }
    setState(() => _submitting = true);
    try {
      final init = await auth.initializePaystackPayment(
          amount: finalTotal, email: email.trim(), purpose: 'DIRECT_ORDER_PAY');
  ...
  Future<void> _showConfirmation(BuildContext context) async {
    ...
    if (!context.mounted || cart.isEmpty) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content:
                  Text('Your cart is empty. Please add a meal and try again.')),
        );
      }
      return;
    }
    if (_fulfilment == 'Schedule pickup' && _scheduledPickup == null) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Choose a pickup time first.')));
      return;
    }
    final points = int.tryParse(_pointsController.text.trim()) ?? 0;
    if (points < 0) {
                ...
                actions: [
                  FilledButton(
                      onPressed: () {
                        Navigator.pop(ctx);
                        if (orderId != null) {
                          Navigator.pushReplacementNamed(ctx, '/order-tracking',
                              arguments: orderId is int
                                  ? orderId
                                  : int.tryParse(orderId.toString()));
                        } else {
