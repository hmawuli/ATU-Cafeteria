...
class _ReferenceStudentScreenState extends State<ReferenceStudentScreen> {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(
          colors: [AppTheme.primary, Color(0xFF0B56A5)],
        ),
  ...
  Widget _featuredVendors(List<User> vendors, bool wide) {
    if (vendors.isEmpty) {
      return const ReferenceCard(
        child: Padding(
          padding: EdgeInsets.all(10),
          child: Text(
            'Featured vendors will appear here when vendors publish their menus.',
          ),
        ),
      );
    }
  ...
  Widget _vendorCard(User vendor) {
    ...
    final foods = provider.allFoodItems
        .where((item) => item.vendorId == vendor.id && item.isAvailable)
        .toList();
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        const Spacer(),
                        StatusPill(vendor.isOpen ? 'Open' : 'Closed'),
                      ],
                    ),
                  ],
                ),
  ...
  Widget _mealGrid(List<FoodItem> items, bool wide) {
    if (!wide) {
      return Column(
        children: items.take(8).map(_meal).toList(),
      );
    }

    final count = items.length > 9 ? 9 : items.length;
              ...
              children: [
                ...
                Positioned(
                  ...
                  child: Container(
                    ...
                    child: Text(
                      item.category.isEmpty ? 'Meal' : item.category,
                      style: TextStyle(
                        color: scheme.primary,
                        fontSize: 10,
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            Text(
              noData
                  ? 'Pull down to refresh and load the latest cafeteria menu.'
                  : 'Try another search term or choose All categories.',
      ...
      child: const Padding(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                'Browse meals freely. Your account is only needed when you are ready to complete checkout.',
                style: TextStyle(
                  color: AppTheme.textDark,
                  height: 1.35,
                ),
  ...
  Widget _vendors(CafeteriaProvider provider, bool wide) {
    final vendors = _vendorDirectory(
      provider,
    ...
    final filtered = vendors.where((vendor) {
      return q.isEmpty ||
          '${vendor.fullName} ${vendor.info}'.toLowerCase().contains(q);
    }).toList();
    ...
    return ListView(
      ...
      children: [
        ...
        if (filtered.isEmpty)
          const ReferenceCard(
            child: Text('No vendors are available right now.'),
          )
        else
          ...filtered.map(
            (vendor) => Padding(
      ...
      child: ReferenceCard(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 6),
            Text(
              '${order.foodName} • Qty: ${order.quantity} • GH₵ ${order.totalPrice.toStringAsFixed(2)}',
              style: const TextStyle(color: AppTheme.textMuted),
            ),
            const SizedBox(height: 16),
    ...
    return ListView(
      ...
      children: [
        ...
        MetricTile(
          ...
          value: 'GH₵ ${user.balance.toStringAsFixed(2)}',
